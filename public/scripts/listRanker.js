...
/**
 *
 * @returns {BetterloxdFilmRecord[]} array of film records from the file
 * @throws {TypeError} when file produces unexpected results
 */
async function getFilmRecordArrayFromFileUpload() {
  ...
  if (file) {
    ...
    let fileContent = await getReadFilePromise(file)
    return readLetterboxdHomemadeList(fileContent)
...
function readListFile() {
  ...
  const file = input.files[0];
  document
    .getElementById("containerContainer")
...
function makeResultDisplay(sortedData) {
  ...
  for (let i = 0; i < sortedData.length; i++) {
    ...
    nextHeader.innerHTML = `${sortedData[i].getRank()}) ${sortedData[
      i
    ].getTitle()} (${sortedData[i].getYear()})`;
  ...
  downloadInOrder.addEventListener("click", () =>
    onRankedListDownload(sortedData, false)
  );
  ...
  downloadReverseOrder.addEventListener("click", () =>
    onRankedListDownload(sortedData, true)
  );
